...
def forward_mega_moe(
    ...
    shared_expert_tp1 = (
        getattr(moe, "_shared_expert_tp1", False)
        or getattr(getattr(moe, "shared_experts", None), "tp_size", None) == 1
    )
    if shared_output is not None and not shared_expert_tp1:
        y.add_(shared_output)
    if (
        not moe._enable_a2a_moe
        and moe.tp_size > 1
        and not should_use_flashinfer_cutlass_moe_fp4_allgather()
    ):
        y = tensor_model_parallel_all_reduce(y)
    if shared_output is not None and shared_expert_tp1:
        y.add_(shared_output)
    return y
...
def _run_mega_routed(
    ...
    if num_tokens > 0:
        ...
        if num_token_non_padded is not None:
            indices = torch.arange(0, num_tokens, device=topk_weights.device)
            topk_weights[indices >= num_token_non_padded, :] = 0
    else:
        topk_ids = None
        topk_weights = None
    ...
    if topk_ids_in.dtype != torch.int32:
        topk_ids_in = topk_ids_in.to(torch.int32)
...
def build_mega_moe_experts_weights(experts) -> None:
    ...
    elif envs.SGLANG_OPT_FIX_MEGA_MOE_MEMORY.get() and use_sm90_fp8_mega:
        # SM90 shares both fp8 weights and block-(128, 128) FP32 scales with the
        # DeepEP grouped-GEMM path. SM90 has no UTCCP scale transpose.
        w13_interleaved, _ = _interleave_l1_weights((w13, w13_sf))
