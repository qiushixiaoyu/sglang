...
def situ_input_sf_group() -> int:
    """Input-activation quant group for the SM90 SiTU FP4 MegaMoE path.

    Mirror of DeepGEMM's get_act_sf_grans_for_mega_moe_sm90_fp4: SiTU
    defaults to the legacy 128/64 activation-scale recipe;
    DG_SM90_FP4_SITU_ACT_GRAN_128_64=0 switches back to the official
    per-32 recipe (accuracy comparison switch).
    """
    import os

    if os.environ.get("DG_SM90_FP4_SITU_ACT_GRAN_128_64", "1") == "0":
        return 32
    return 128
...
def run_sm90_mega_routed(
    ...
    if activation == "situ" and not use_fp4:
        raise RuntimeError(
            "SM90 MegaMoE SiTU is currently supported only with FP4 weights"
        )
    ...
    deep_gemm.mega_moe_pre_dispatch_sm90(
        ...
        group_size=situ_input_sf_group() if activation == "situ" else 128,
