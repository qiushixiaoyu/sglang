...
def _reference_pre_dispatch(
    ...
    buf_topk_idx = torch.empty((padded_max, top_k), dtype=torch.int64, device=x.device)
...
def _run_kernel(
    ...
    from sglang.jit_kernel.dsv4 import mega_moe_pre_dispatch_sm90
...
def test_sm90_pre_dispatch_matches_reference(
    ...
    topk_weights = torch.randn(num_tokens, top_k, dtype=torch.float32, device=device)
    ...
    if num_tokens > 0:
        ...
        def _dequant(buf_x, buf_sf):
            return buf_x[:num_tokens].float().reshape(
                num_tokens, num_groups, 128
